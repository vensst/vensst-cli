...
### 安装
\`\`\`shell
${answers.installType} run install
\`\`\`
### 运行
\`\`\`shell
${answers.installType} run serve
\`\`\`
### 编译
\`\`\`shell
${answers.installType} run build
\`\`\`
        `);
...
const consoleStatus=async function (projectName, answers) {
  ...
  if(answers.isAutoInstall){
    console.log(`${chalk.green(`  ${answers.installType} install`)}`);
  }
  console.log(`${chalk.green(`  ${answers.installType} run serve`)}`);
